 ...
 */
  ...
  * 
  */
const float EXPECTED_MAG = 4050.0;
const int ACCEL_SAMPLES = 7; // number of samples from accelerometer to keep and use
const int WEIGHT_INERTIA = 5; // how weighted the samples are
        // == 0   ==>   equal weights
        // >= 0   ==>   more trust to vectors closer to gravity in magnitude
        // <= 0   ==>   more trust to vectors farther from gravity in magnitude
              /* avoid final case */
const float ANGLE_SCALE_FACTOR = 1.2; // how much to compensate when tilting ( !> 1 )
const float XY_SCALE_FACTOR = 2.0 / 1000;
              
#define P_GAIN_XY 0.1
#define I_GAIN_XY 0.2
#define D_GAIN_XY 0
...
const float MAX_D_DISP = 3.0 / 50; // assumes 3 inches per 50 cycles (50Hz runtime) 
...
// Debug arrays for monitoring PID output during runtime
float pErr_xy[2] = {0, 0};
float iErr_xy[2] = {0, 0};
float dErr_xy[2] = {0, 0};
float pErr_tilt[2] = {0, 0};
float iErr_tilt[2] = {0, 0};
float dErr_tilt[2] = {0, 0};
...
void selfBalance() {
  ...
  for (int i = 0; i < 50; i++) {
    ...
    // Print messages every second to monitor system status
    if (i == 0) {
      ...
      Serial.println("X-tilt\tY-tilt");
      Matrix.Print(pErr_tilt, 1, 2, "P");
      Matrix.Print(iErr_tilt, 1, 2, "I");
      Matrix.Print(dErr_tilt, 1, 2, "D");

      Serial.println();
      Serial.println("X-comp\tY-comp");
      Matrix.Print(pErr_xy, 1, 2, "P");
      Matrix.Print(iErr_xy, 1, 2, "I");
      Matrix.Print(dErr_xy, 1, 2, "D");
...
void calcOrientation() {
  ...
  for (int i = ACCEL_SAMPLES - 1; i > 0; i--) {
    for (int j = 0; j < 3; j++) {
      accelData[i][j] = accelData[i - 1][j];
    }
    accelWeights[i] = accelWeights[i - 1];
  }
  ...
  accelWeights[0] = pow( EXPECTED_MAG / calcVecMag(temp), WEIGHT_INERTIA);
  ...
  // Calculate the angular offset of the vector
  // In this case, we want the platform to remain flat even if 
  // there is lateral acceleration
  // we calculate the angular offset caused by translational acceleration
  // (should cause platform to remain flat regardless of acceleration)
  float theta_x = asin(accel[0]/magnitude);
  float theta_y = asin(accel[1]/magnitude);
...
void calcServoAngles() {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    if (angles[i] > 75*deg2rad) {
      angles[i] = 75*deg2rad;
    } else if (isnan(angles[i])) {
      angles[i] = 75*deg2rad;
    }
  ...
  // Use angle data to calculate pulse widths
    ...
    Matrix.Print(angles, 1, 6, "angles");
}
...
void xy_PID() {
  float err, pErr, dErr, vRef, vAct, errSum;
  ...
  for (int i = 0; i < 2; i++) {
    // Reference
    vRef = accel[i] * XY_SCALE_FACTOR; // 4100 ~= gravity in gyroscope land
    vAct = tDisp[i];

    //Matrix.Print(accel, 1, 3, "accel");
    ...
    // Adjust X
    errSum = pErr + iErr[i] + dErr;
    if (errSum > MAX_D_DISP) {
      errSum = MAX_D_DISP;
    } else if (errSum < -MAX_D_DISP) {
      errSum = -MAX_D_DISP;
    }
    tDisp[i] += errSum;
    ...
    if (i == 2) {
      Serial.print("P: " + (String)(pErr));
      Serial.print("I: " + (String)(iErr[0]));
      Serial.println("D: " + (String)(dErr));
    }
  }
}
