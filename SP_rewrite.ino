...
// #define-d because these are only referenced one time during set-up
#define   ANGLE_DIFF_PLAT   110*deg2rad
#define   ANGLE_DIFF_BASE   50*deg2rad
const float BETA[]      = {pi/3, 1*pi/3, 3*pi/3, 3*pi/3, 5*pi/3, 5*pi/3};
float alpha_o;
...
void setup() {
  ...
  servo[0].attach(3);     // attaches the servo on pin 3 to the servo object
    ...
    /*
     *  Initialize platform displacement vector
     */
    tDisp[2] = h_o;
...
void calcServoAngles() {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    float N = 2*ARM_LEN*(cos(BETA[i])*legVectors[i][0] + sin(BETA[i])*legVectors[i][1]);
