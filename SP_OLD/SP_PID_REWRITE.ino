 ...
 */
#define PID_CONTROL
#define PROPORTIONAL_GAIN   1.2
#define INTEGRAL_GAIN       1
#define DERIVATIVE_GAIN     0
#define RUN_TIME      19990  // expected algorithm run-time in microseconds
...
const float MAX_ANGLE = 50*deg2rad;
...
void selfBalance() {
  ...
  while (timeElapsed - timeRef < RUN_TIME) {
    timeElapsed = micros();
  }
}
...
void sp_PID() {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    float pErr = k_P * (legRef[i] - legLength[i]);
...
void calcOrientation() {
  ...
  // PLATFORM ROLL  --- NORM YAW
    orientation[0] = asin(-unit[1])               + TILT_OFFSET[0];

    // PLATFORM PITCH --- NORM ROLL
    orientation[1] = atan2(unit[0], unit[2])      + TILT_OFFSET[1];
    ...
    for (int i = 0; i < 2; i++) {
      ...
      if (abs(orientation[i]) > MAX_ANGLE) {
        disableProg = TRUE;
      } 
      // if we're leaving the linear region, raise that flag
      else if (abs(orientation[i]) > LINEAR_ANGLE) {
        inLinear = FALSE;;
      } 
      // if we're in the linear region, set flags
      else if (abs(orientation[i]) < LINEAR_ANGLE - 10*deg2rad) {
        inLinear = TRUE;
      } 
    }
 ...
 void setupGyro() 
{
  ...
  imu.settings.accel.highResBandwidth = 2; // something something low-pass filter for accel?
